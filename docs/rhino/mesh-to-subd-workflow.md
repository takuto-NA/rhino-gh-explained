...
Q: 3Dスキャンなどで得られた既存のメッシュデータ（原型）をそのまま使わず、あえてSubDに「作り直す」目的は何ですか？

A: 製造や設計に適さない「汚いメッシュ（不規則な三角形の集まり）」は、穴を塞いだり法線を整えたりする修復作業に時間をかけるよりも、**編集可能な四角形トポロジに作り直す（再構築する）**方が、結果として工数を削減でき、その後の設計変更にも柔軟に対応できるためです。Rhino 8では、`QuadRemesh` を起点としたSubDワークフローが最も効率的です。
...
- **トポロジの整理**: メッシュ特有の「悪い三角形」や「過密な頂点」をリセットし、**四角面中心の綺麗な流れ**を自動生成できる。
