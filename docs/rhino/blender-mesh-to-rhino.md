...
GHで参照に使用する場合は、**見た目がほぼ同じでポリゴン数が少ない方が有利**です。

- 目安: 動作する密度まで軽量化する（細部は後段の曲線/面で再構築する前提）
...
合っていない場合は、単位問題を確認し、「Blender側のUnits」または「書き出しスケール」を見直します。
...
- メッシュ修復に時間をかけるより、`QuadRemesh` → SubD編集へ切り替える方が効率的です
