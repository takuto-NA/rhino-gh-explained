...
A: アニマトロニクス外装は「基本1mm」でも、首周り・コネクタ周り・分割フランジなどで**局所的に厚く**する必要があります。ここでのポイントは、**最初から可変厚みにしない**ことです。

**Q: 安全な順序は？**

A: 以下の順序で進めます：
1. **均一厚みで「閉じた殻」を作る**（初期は1mm相当）
...
A: どの表現（Mesh/SubD/Brep）で進めるかで手段が変わります：
- **Brep（NURBSソリッド）**: `Shell` が最も効率的
- **開いた面（サーフェス）**: `OffsetSrf`（必要なら Solid=Yes）
- **SubD/メッシュ**: 「殻を作れる表現」に移行する
...
**Q: メッシュを厚くしたい（最終STL前提）場合は？**

A: `OffsetMesh`を使用します。メッシュのまま加工する場合は `OffsetMesh` を使います：
- まず `CheckMesh` で穴/非多様体/自交差を確認してから実行する
- 1mmが破綻する場合は、先に分割してから部分ごとにオフセットする

**Q: SubDの扱いは？**

A: SubDは造形に強い一方、"製造用の殻/当たり面"を作る段階では、次のどちらかが安定です：
- `ToNURBS` して Brep に寄せ、`Shell` / `OffsetSrf` で厚みを作る
- `ToMesh` してメッシュに寄せ、`OffsetMesh` で厚みを作る

## スクリプト例

**Q: RhinoCommon（C#）でBrepを1mmオフセットしてソリッド化する方法は？**

A: Rhino上での `OffsetSrf` / `Shell` と同じ発想を、RhinoCommonで行う例です。厚み方向を反転したい場合は距離を負にします（-1 など）。
...
**Q: Grasshopper（Python）でRhinoCommonを使って1mmオフセットする方法は？**

A: GHのPythonコンポーネントで、入力Brepを1mm厚にします。
...
**Q: 強曲率部で自己交差が発生する場合の対策は？**

A: その領域を先に分割し、局所ごとに処理します。

**Q: 薄肉のまま大開口を作って割れる（首周り）場合の対策は？**

A: 開口縁にフランジ/リブを前提で設計します。

**Q: 可変厚みを先に入れて、後段の穴あけ/分割で破綻する場合の対策は？**

A: 先に均一殻→設計（分割/穴）→最後に補強の順序で進めます。
