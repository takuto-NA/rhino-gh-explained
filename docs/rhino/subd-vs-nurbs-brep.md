...
A: Rhino 8で頻出する **SubD** を「何が嬉しいのか／何が苦手なのか」を、NURBSとBrep（境界表現）と対比して整理します。  
有機形状（キャラクター、フィギュア、プロップ、外装など）では、**SubDで造形 → 必要箇所だけNURBS/Brepで「当たり面/機構」を作る**方法が実用的です。

**Q: NURBS / Brep / SubD の違いは？**

A: これらは "同じレイヤー" の概念ではありません。以下のように整理します：
...
A: NURBSの曲線（概念）は、Bスプライン基底関数 \(N_{i,p}(u)\) と制御点 \(P_i\) の重み付き合成です。
...
A: **Brep = 境界表現**です。「このソリッドは、これらの面が閉じてできています」という構造を持ちます。
...
A: 「BrepだからNURBSとは別物」ではなく、**多くのBrepはNURBS曲面で構成される**、という理解が正確です。

## SubD（Subdivision Surface）とは

**Q: SubDとは何ですか？**

A: SubDは、**粗いケージ（主に四角面）**を反復的に細分化して、滑らかな極限曲面（limit surface）を得る表現です。Rhino 8はこのSubDを "CAD寄り" に使えるように統合しています。

**Q: SubDとNURBSの編集対象の違いは？**

A: 編集対象が「制御点」ではなく「ケージ」である点が異なります：
- NURBS: 制御点が多くなりがち（面構成と連続性も管理対象）
- SubD: 低解像度のケージを動かすだけで、滑らかな曲面が付いてくる

**Q: SubDの数学的な要点は？**

A: 一般的なSubD（Catmull–Clark）は、細分化ステップで「面点」「辺点」「頂点点」を更新していき、反復回数を増やすと極限曲面に収束します。
