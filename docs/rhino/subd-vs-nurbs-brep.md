...
アニマトロニクス外装のような有機形状では、**SubDで造形 → 必要箇所だけNURBS/Brepで“機構”を作る**のが現実的に強いです。
...
結論として、\n+**NURBSは“曲面の数式”**、\n+**Brepは“ソリッドを持つための枠組み”**、\n+**SubDは“有機形状を素早く作るための曲面表現”**、という役割分担です。
...
**Brep = 境界表現**です。\n+「このソリッドは、これらの面が閉じてできています」という構造を持ちます。
...
- `Shell` / `BooleanDifference` など **ソリッド演算が強い**

### 注意点

「BrepだからNURBSとは別物」ではなく、\n+**多くのBrepはNURBS曲面で構成される**、という理解が正確です。

## SubD（Subdivision Surface）とは

SubDは、**粗いケージ（主に四角面）**を反復的に細分化して、滑らかな極限曲面（limit surface）を得る表現です。\n+Rhino 8はこのSubDを “CAD寄り” に使えるように統合しています。
...
一般的なSubD（Catmull–Clark）は、\n+細分化ステップで「面点」「辺点」「頂点点」を更新していき、\n+反復回数を増やすと極限曲面に収束します。

特に重要なのが **特異点（extraordinary vertex）** です。\n+四角格子でない頂点（次数≠4）の周りでは、\n+極限曲面は “NURBSのように単純なパッチの集合” にはなりません。\n+これが「SubD→NURBS変換で面が増える」主要因です。
...
- そのまま“ソリッド演算の土台”にするより、\n+  **当たり面/機構部はBrepで作って合流**する方が壊れにくい

## 使い分け（アニマトロニクス外装の推奨）

- **外装の造形（顔/耳/頬/額）**: SubD中心\n- **厚み・分割・穴・機構との当たり面**: Brep（NURBS）中心\n- **変換**:\n+  - SubD→Brep（`ToNURBS`）は「機構当たり面を作る/ソリッド演算したい」タイミングで限定的に\n+  - 最終出力はSTL/3MFなので、最後は“プリント可能な殻”になっているかをスライサーで検証
