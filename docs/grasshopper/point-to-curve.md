...
| 要件（最初に決めること） | 第一候補 | 何を保証/最適化しているか |
| --- | --- | --- |
| **すべての点を必ず通る** | **Interpolate（補間）** | 点の通過を保証（ただしノイズでうねりやすい） |
| **ノイズ込みで滑らかにしたい** | **Fit Curve（近似）** | 誤差内で“もっともらしさ”を最適化（許容誤差で調整） |
| **角を残したい（折れ線）** | **Polyline** | 点を直線で接続（丸めない） |
| **LINE/ARCだけで再現したい（DXF/CAM）** | **Fit Line / Fit Arc（＋セグメント分割）** | 幾何要素（Line/Arc）に落とす（加工/交換向け） |
| **制御点でデザインしたい** | **Nurbs Curve（Control Points）** | 点は通さず“引っ張る”重みとして機能 |
...
| チェック | 何を見るか |
| --- | --- |
| **点の意味** | 全点通過が必要か（補間）／誤差内で良いか（近似） |
| **点の品質** | 重複点、ノイズ、リスト順序（並び）の正しさ |
| **欲しい曲線の性質** | Closedの要否、接線制御の要否、角を残す必要の有無 |
...
| やりたいこと | コンポーネント | 入力 | 出力 |
| --- | --- | --- | --- |
| linspace相当（開始〜終了をN分割） | `Range` | `Domain`（開始〜終了）, `Steps`（分割数） | 端点を含む等間隔な数値リスト |
| 一定ステップ（開始, 開始+増分, …） | `Series` | `Start`（開始）, `Step`（増分）, `Count`（個数） | 等差数列 |
...
Q: 「Simplifyってコンポーネントなかったっけ？」（`SimplifyCrv` との違いは？）

A: Grasshopperでよく出てくる **`Simplify` は基本的に「Data Treeの階層を簡略化する操作」** です（パラメータ右クリックの `Simplify` や、`Simplify Tree` コンポーネント）。
一方、Rhino側の **`SimplifyCrv` は「曲線のセグメントを直線・円弧に置き換える（＋連続する同一直線/同一円の結合など）コマンド」** で、目的も結果も別物です。
曲線を直線・円弧ベースに“単純化”したい場合は `SimplifyCrv` を使います（仕様はRhinoヘルプ参照: `https://docs.mcneel.com/rhino/7/help/en-us/commands/simplifycrv.htm` ）。

| 失敗パターン | 何が起きるか | 対策 |
| --- | --- | --- |
| **点の順序が不適切** | 自己交差・蛇行 | 生成順を管理／`Sort Along Curve` 等を定義に含める |
| **重複点/過密点** | 曲率が局所的に急増し不具合 | `Cull Duplicates`／必要なら間引き（Downsampling） |
| **ノイズ点列にInterpolate** | 不要な振動（波打ち） | `Fit Curve`に切替／平滑化してから補間 |
| **Data Treeが意図と違う** | 意図しない曲線が複数生成 | `Simplify` / `Flatten` 等で階層を整理（1リスト→1曲線） |
