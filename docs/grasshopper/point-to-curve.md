...
Grasshopperでは、コンポーネントの選択によって結果が大きく変化します。本ページでは、**点列からCurveを生成する際の主要機能の仕組み（何を保証し、何を最適化しているか）**を、実務判断に必要な範囲で整理します。
...
点列をDXFフォーマットでCADに読み込めるようにする場合、以下の2つのアプローチがあります。

#### アプローチ1: 細かい直線セグメントで近似する
...
#### アプローチ2: 直線（LINE）と円弧（ARC）のセグメントで近似する（推奨）
