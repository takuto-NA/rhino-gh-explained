...
運用方針: 「造形はGHで」「仕上げはRhinoで」になる場合は、ベイク地点を意図的に設定すると運用が安定します。
...
両方を同時に完璧に保つのは困難なため、**GHを主履歴**として設計し、RhinoのHistoryは補助的に使用する運用を推奨します。
